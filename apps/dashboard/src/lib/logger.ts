const colors = {
  reset: "\x1b[0m",
  bright: "\x1b[1m",
  dim: "\x1b[2m",
  underscore: "\x1b[4m",
  blink: "\x1b[5m",
  reverse: "\x1b[7m",
  hidden: "\x1b[8m",
  
  black: "\x1b[30m",
  red: "\x1b[31m",
  green: "\x1b[32m",
  yellow: "\x1b[33m",
  blue: "\x1b[34m",
  magenta: "\x1b[35m",
  cyan: "\x1b[36m",
  white: "\x1b[37m",
  
  bgBlack: "\x1b[40m",
  bgRed: "\x1b[41m",
  bgGreen: "\x1b[42m",
  bgYellow: "\x1b[43m",
  bgBlue: "\x1b[44m",
  bgMagenta: "\x1b[45m",
  bgCyan: "\x1b[46m",
  bgWhite: "\x1b[47m"
};

export const logger = {
  info: (message: string, data?: any) => {
    console.log(
      `${colors.cyan}[INFO]${colors.reset} ${message}`,
      data ? `\n${colors.dim}${JSON.stringify(data, null, 2)}${colors.reset}` : ''
    );
  },
  
  success: (message: string, data?: any) => {
    console.log(
      `${colors.green}[SUCCESS]${colors.reset} ${message}`,
      data ? `\n${colors.dim}${JSON.stringify(data, null, 2)}${colors.reset}` : ''
    );
  },
  
  warn: (message: string, data?: any) => {
    console.log(
      `${colors.yellow}[WARN]${colors.reset} ${message}`,
      data ? `\n${colors.dim}${JSON.stringify(data, null, 2)}${colors.reset}` : ''
    );
  },
  
  error: (message: string, error?: any) => {
    console.error(
      `${colors.red}[ERROR]${colors.reset} ${message}`,
      error ? `\n${colors.dim}${JSON.stringify(error, null, 2)}${colors.reset}` : ''
    );
  },
  
  auth: (message: string, data?: any) => {
    console.log(
      `${colors.magenta}[AUTH]${colors.reset} ${message}`,
      data ? `\n${colors.dim}${JSON.stringify(data, null, 2)}${colors.reset}` : ''
    );
  },
  
  api: (method: string, path: string, data?: any) => {
    console.log(
      `${colors.blue}[API]${colors.reset} ${colors.bright}${method}${colors.reset} ${path}`,
      data ? `\n${colors.dim}${JSON.stringify(data, null, 2)}${colors.reset}` : ''
    );
  },

  group: (title: string) => {
    console.group(`${colors.bright}${title}${colors.reset}`);
  },

  groupEnd: () => {
    console.groupEnd();
  }
}; 